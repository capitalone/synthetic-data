...
class TestTabularGenerator(unittest.TestCase):
    ...
    #     self, mock_generate_uncorrelated_cols
    # ):
    ...
    #     # checking a list of dictionaries.
    #     # this will check if generated params is correct

    #     # mock_generate_uncorrelated_cols is a func that returns
    #     # a dataframe of synthetic data
    #     # need to check if convert_data_to_df -> check for args
    #     mock_generate_uncorrelated_cols.assert_called_once()

    #     # we want to check if convert_data_to_df is called with the correct params
    #     for i in range(len(expected_columns_to_generate)):
    #         for key in mock_generate_uncorrelated_cols[i].keys():
    #             if isinstance(mock_generate_uncorrelated_cols[i][key], list):
    #                 self.assertTrue(
    #                     set(mock_generate_uncorrelated_cols[i][key]).issubset(
    #                         expected_columns_to_generate[i][key]
    #                     )
    #                 )
    #             else:
    #                 self.assertEqual(
    #                     mock_generate_uncorrelated_cols[i][key], expected_columns_to_generate[i][key]
    #                 )
    ...
    def test_generate_uncorrelated_column_data(
        ...
        # check if each generator was called with the correct params
        # iterate through the mocked random generators
        for i, generator in enumerate(random_generators):
            # get the args_list for each generator
            call_args_list = generator.call_args_list
            # expected_calls[i] is a mock.call(expected_params),
            # and this corresponds to generator for each generator in generators.
            # this wont work since list params can differ
            # self.assertEqual(call_args_list, expected_calls[i])

            # possible solution to randomized list parameters:
            # call_args_list returns (*args, **kwargs)
            # it's prob always kwargs so:
            # call_args_list[0] -> {param_name:var}
            # iterate through this dictionary of params
            # and basically check if the value is a list, if it is,
            # assertCountEqual for that
            for j, key in enumerate(call_args_list):
                # if the current value is a list, compare that with the expected
                # expected is a mock call -> ({param:value})
                if isinstance(call_args_list[key], list):
                    # get ith expected call list. format:
                    # expected_calls = [
                    #     [
